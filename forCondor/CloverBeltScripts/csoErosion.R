...
# #partition data
# set.seed(123)
# splitR <- initial_split(csoR, strata = Erosion)
# trainR <- training(splitR)
# testR <- testing(splitR)
# 
# cso_recR <- recipe(Erosion ~ ., data = trainR)
# 
...
#   collect_metrics()
# write.csv(metricsR, "csoErosionMetricsCloverBelt_withR.csv", row.names = FALSE, quote = FALSE)
# 
# #choose best model
...
#   
# 
# saveRDS(modR, "csoErosionCloverBelt_withR.rds")
# 
# testRMSE_r <- modR %>%
#   predict(testR) %>%
#   bind_cols(testR) 
...
metrics <- regular_res %>%
  collect_metrics()
write.csv(metrics, "csoErosionMetricsCloverBelt.csv", row.names = FALSE, quote = FALSE)
...
saveRDS(mod, "csoErosionCloverBelt.rds")
