...
# #partition data
# set.seed(123)
# splitR <- initial_split(dlR, strata = Erosion)
# trainR <- training(splitR)
# testR <- testing(splitR)
# 
# dl_recR <- recipe(Erosion ~ ., data = trainR)
# 
...
#   collect_metrics()
# write.csv(metricsR, "dlErosionMetricsCloverBelt_withR.csv", row.names = FALSE, quote = FALSE)
# 
# #choose best model
...
#   
# 
# saveRDS(modR, "dlErosionCloverBelt_withR.rds")
# 
# testRMSE_r <- modR %>%
#   predict(testR) %>%
#   bind_cols(testR) 
...
dl <- full %>%
  ...
  filter(OM < 20) %>%
  mutate_if(is.character, as.factor) %>%
  select(c(Erosion, density, total_DM_lbs, slope, slopelenusle.r, sand, silt, clay, k)) %>%
  distinct() %>%
  droplevels()
...
#partition data
set.seed(123)
split <- initial_split(dl, strata = Erosion)
train <- training(split)
test <- testing(split)

dl_rec <- recipe(Erosion ~ ., data = train)
...
metrics <- regular_res %>%
  collect_metrics()
write.csv(metrics, "dlErosionMetricsCloverBelt.csv", row.names = FALSE, quote = FALSE)
...
saveRDS(mod, "dlErosionCloverBelt.rds")
