...
best.guess <- round(mean(ccErosion$Erosion),2) #4.74
RMSE.baseline <- round(sqrt(mean((best.guess-train$Erosion)^2)),2) #6.18
...
rf50 <- randomForest(Erosion ~ ., data = train, ntree = 50, importance = TRUE, do.trace = TRUE)
rf50 #99.97
pred_rf50 <- predict(rf50, test)
RMSE.50 <- sqrt(mean((pred_rf50 - test$Erosion)^2)) #0.62
plot(pred_rf50 ~ test$Erosion, xlab = "Erosion", ylab = "Predicted Erosion", main = "trees: 50, mtry: 3")

rf50.5 <- randomForest(Erosion ~ ., data = train, ntree = 50, mtry = 5, importance = TRUE, do.trace = TRUE)
rf50.5 #99.86
pred_rf50.5 <- predict(rf50.5, test)
RMSE.50.5 <- sqrt(mean((pred_rf50.5 - test$Erosion)^2)) #0.20
plot(pred_rf50.5 ~ test$Erosion, xlab = "Erosion", ylab = "Predicted Erosion", main = "trees: 50, mtry: 5")

rf75.6 <- randomForest(Erosion ~., data = train, ntree = 75, mtry = 6, importance = TRUE, do.trace = TRUE)
rf75.6 #99.94
pred75.6 <- predict(rf75.6, test)
RMSE75.6 <- sqrt(mean((pred75.6 - test$Erosion)^2)) #0.13
plot(pred75.6 ~ test$Erosion, xlab = "Erosion", ylab = "Predicted Erosion", main = "trees: 75: mtry 6")
plot(rf75.6)
...
stepForward$anova
length(stepForward$coefficients) #141
stepForward.model <- train(PI ~ Erosion + cover + tillage + slope + Contour + initialP + 
                             ...
                             OM:slopelenusle.r + totalP2O5_lbs:silt + initialP:silt + 
                             silt:k + OM:silt + initialP:LSsurgo + tillage:total_DM_lbs + 
                             Erosion:cover + tillage:initialP + tillage:slope + slopelenusle.r:total.depth + 
                             initialP:total.depth + totalP2O5_lbs:total_DM_lbs + cover:initialP + 
                             cover:totalP2O5_lbs + Erosion:total.depth + initialP:slopelenusle.r + 
                             cover:tillage + Erosion:tillage + tillage:k + slope:slopelenusle.r + 
                             slope:LSsurgo + Contour:initialP + initialP:total_DM_lbs + 
                             totalP2O5_lbs:LSsurgo + Erosion:LSsurgo + cover:k + cover:slope + 
                             total_DM_lbs:k + OM:total_DM_lbs + Contour:slopelenusle.r + 
                             Erosion:slopelenusle.r + Erosion:slope + tillage:slopelenusle.r + 
                             tillage:LSsurgo + initialP:OM + Contour:total_DM_lbs + total_DM_lbs:total.depth + 
                             silt:total.depth + Erosion:k + tillage:Contour + tillage:total.depth + 
                             tillage:totalP2O5_lbs + Contour:OM + tillage:OM + slope:k + 
                             Erosion:Contour + cover:Contour + slope:Contour + cover:slopelenusle.r + 
                             cover:LSsurgo + OM:total.depth + k:total.depth + OM:totalP2O5_lbs + 
                             totalP2O5_lbs:k + cover:total.depth + OM:LSsurgo + slope:OM + 
                             slope:totalP2O5_lbs + totalP2O5_lbs:slopelenusle.r + slopelenusle.r:LSsurgo + 
                             k:LSsurgo + slopelenusle.r:k + silt:LSsurgo + slope:silt + 
                             slopelenusle.r:silt + total.depth:LSsurgo + slope:total.depth + 
                             totalP2O5_lbs:total.depth + cover:silt + slope:initialP + 
                             cover:total_DM_lbs + Contour:k + tillage:silt + initialP:totalP2O5_lbs + 
                             Contour:total.depth + total_DM_lbs:silt,
...
## investige extra tillage (sn vs sc ... the total between the two is the full set)
cc <- snap %>% 
  filter(crop == "cc",
         cover == "cc" 
         | cover == "gcds"
  ...
  mutate(Contour = as.factor(Contour),
         manureSynth = as.factor(paste(ManureApp, SyntheticApp))) %>%
  mutate_if(is.character, as.factor) %>%
  droplevels()
